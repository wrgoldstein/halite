...
class GameMap:
    ...
    def less_naive_navigate(self, ship, destination):
        unsafe_moves = self.get_unsafe_moves(ship.position, destination)
        safe_moves = [m for m in unsafe_moves if not self[ship.position.directional_offset(m)].is_occupied]
        if len(safe_moves):
            safe_direction = random.choice(safe_moves)
            pos = ship.position.directional_offset(safe_direction)
    ...
    def dangerous_navigate(self, ship, destination):
        unsafe_moves = self.get_unsafe_moves(ship.position, destination)
        safe_moves = []
        all_cardinals = Direction.get_all_cardinals()
        for cardinal in all_cardinals:
            if ship.position.directional_offset(cardinal) == destination:
                return cardinal
        return self.less_naive_navigate(ship, destination)
    ...
    def evasive_maneuvers(self, ship):
        surrounds = ship.position.get_surrounding_cardinals()
        safe_positions = [p for p in surrounds if not self[p].is_occupied]
        if len(safe_positions):
            pos = random.choice(safe_positions)
            cardinalities = [c for c in self._get_target_direction(ship.position, pos) if c is not None]
            direction = random.choice(cardinalities)
            self[ship.position.directional_offset(direction)].mark_unsafe(ship)
        else:
            direction = 'o'
            self[ship.position].mark_unsafe(ship)
